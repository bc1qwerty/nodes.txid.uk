...
function renderTopNodes(nodes){
  ...
  el.innerHTML=(nodes||[]).slice(0,20).map((n,i)=>`
    <div class="node-row" onclick="loadNodeDetail('${n.public_key}')">
      <span class="node-rank">#${i+1}</span>
      <span class="node-alias">${n.alias||n.public_key.slice(0,16)+'…'}</span>
...
async function searchNode(){
  ...
  try{
    ...
    if(nodes.length===1){await loadNodeDetail(nodes[0].public_key);return;}
    renderTopNodes(nodes);
  ...
  try{
    ...
    el.innerHTML=`
      ...
      <div class="nd-grid">
        ...
        <div class="nd-item"><div class="nd-key">채널 수</div><div class="nd-val">${(n.active_channel_count||0).toLocaleString()} 활성</div></div>
        <div class="nd-item"><div class="nd-key">국가</div><div class="nd-val">${n.country?.en||'알 수 없음'}</div></div>
        <div class="nd-item"><div class="nd-key">첫 등장</div><div class="nd-val">${n.first_seen?new Date(n.first_seen*1000).toLocaleDateString('ko-KR'):'—'}</div></div>
        <div class="nd-item"><div class="nd-key">마지막 업데이트</div><div class="nd-val">${n.updated_at?new Date(n.updated_at*1000).toLocaleDateString('ko-KR'):'—'}</div></div>
      </div>`;
